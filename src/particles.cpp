...
#include <unistd.h>  // for use with sleep()
...
#define NUM_PARTICLES  576000 //768000 //128000 //160000 //17000
//#define MAX_NUM_PARTICLES 350000 //22000
#define MAX_NUM_PARTICLES 1000000 //22000
...
const uint width = 640, height = 240;
...
void runBenchmark(int iterations, char *exec_path) //, FILE *& flog)
{
    printf("Run %u particles simulation for %d iterations...\n\n", numParticles, iterations);
    fprintf(flog,"Run %u particles simulation for %d iterations...\n\n", numParticles, iterations);
    cudaDeviceSynchronize();
    sdkStartTimer(&timer);

    for (int i = 0; i < iterations; ++i)
    {
        psystem->update(timestep, flog);
    }
    ...
    fprintf(flog,"particles, Throughput = %.4f KParticles/s, Time = %.5f s, Size = %u particles, NumDevsUsed = %u, Workgroup = %u\n",
            (1.0e-3 * numParticles)/fAvgSeconds, fAvgSeconds, numParticles, 1, 0);
...
/*
//    if (num_iter>1000) {
    // RMK: Save image to file
	// Make the BYTE array, factor of 3 because it's RBG.
	//BYTE* pixels = new BYTE[ 3 * width * height];
	BYTE* pixels = new BYTE[ 3 * 640 * 240];
	//GL_UNSIGNED_BYTE = new GL_UNSIGNED_BYTE[ 3 * 640 * 480];

	glReadPixels(0, 0, 640, 240, GL_RGB, GL_UNSIGNED_BYTE, pixels);

	// Convert to FreeImage format & save to file
	FIBITMAP* image = FreeImage_ConvertFromRawBits(pixels, 640, 240, 3 * 640, 24, 0x0000FF, 0xFF0000, 0x00FF00, false);

	stringstream ss;
	ss << setw(4) << setfill('0') << num_iter;
	framefile = "/home/rkeedy/PartIm2/testim"+ss.str()+".bmp";
	FreeImage_Save(FIF_BMP, image, framefile.c_str(), 0);
...
main(int argc, char **argv)
{
    ...
    printf("Opening output log file\n");
    flog = fopen("/home/keedy/CFD/BuoyantStrumJet85-big-refine-lighter-3d/logfile.txt","w");
...
//	char Rfile[] = "/home/rkeedy/Dropbox/CFD/BuoyantStrumJet85-big-refine-lighter/RVert.txt";
	...
	numVelNodes.y = 1;
	// THESE NEED TO BE THE SAME AS THE VALUES FOR NUM_CELLS_X,_Y,_Z in particles_kernel.cuh
	numCells.x = 40; //80; //47; //24; //29;
	numCells.y = 40; //1;
	numCells.z = 40; //160; //188; //95; //88;
	// sync with particle_system.cpp
	numParticles = numCells.x*numCells.y*numCells.z*9; //avgnumparticles = 40
	printf("Maybe I can get this far\n");
	srand( time( NULL ) );
