...
/*    void setCellParameters(CellParams *hostParams)
    ...
    void integrateSystem(float *pos,
                         ...
                         uint numParticles)
    {
    	printf("Thrusting in integrateSystem\n");
        thrust::device_ptr<float4> d_pos4((float4 *)pos);
        thrust::device_ptr<float3> d_vel3((float3 *)vel);
        //thrust::device_ptr<float2>  d_scalar((float2 *)scalar);

        printf("Thrusting again in integrateSystem\n");
        thrust::for_each(
        	thrust::make_zip_iterator(thrust::make_tuple(d_pos4, d_vel3)),
            //thrust::make_zip_iterator(thrust::make_tuple(d_pos4, d_vel4, d_scalar)),
        	thrust::make_zip_iterator(thrust::make_tuple(d_pos4+numParticles, d_vel3+numParticles)),
...
//#endif
...

    {
     ...
     //   checkCudaErrors(cudaBindTexture(0, oldVelTex, sortedVel, maxNumParticles*sizeof(float3)));
        ...
        // execute the kernel
        advectD<<< numBlocks, numThreads >>>((float3 *)newVel,
                                              (float4 *)sortedPos,
                                             // (float4 *)sortedVel,
                                              gridParticleIndex,
                                              cellStart,
