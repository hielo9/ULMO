...
struct integrate_functor
{
    ...
    void operator()(Tuple t)
    {
        ...
        if (pos.y > params.worldSize.y+params.worldOrigin.y)
        {
        	delta = pos.y - (params.worldSize.y+params.worldOrigin.y);
        	pos.y = params.worldSize.y+params.worldOrigin.y - delta;
        }

        if (pos.y < params.worldOrigin.y)
        {
        	delta = params.worldOrigin.y - pos.y;
        	pos.y = params.worldOrigin.y + delta; //params.R[0];
        }
...
*/
        ...
        thrust::get<1>(t) = vel; //make_float3(vel); //, velData.w);
        //thrust::get<2>(t) = make_float2(scalar, scalarData.w);
        //printf("End of integrate kernal\n");
    }
};
...
__device__ int3 calcVelGridPos(float3 p)
{
    ...
    gridPos.y = 0;
    gridPos.z = 0;
    radius = sqrt(powf(p.x,2.)+powf(p.y,2.));
    for (int i=0;i<params.numVelNodes.x;i++) {
    	if (params.R[i+1]>=radius) {
    		gridPos.x = i;
    		break;
    	}
    }
...
__device__ int3 calcGridPosW(float3 p,float w)
{
    ...
    gridPos.y = floor((p.y - params.worldOrigin.y+eps) / params.cellSize.y);
    ...
    if (gridPos.y>params.numCells.y-1) {
    	gridPos.y = params.numCells.y-1;
    	printf("HEY NOWy; can't do this: %d %d %15.12f %15.12f %d %d %15.12f %15.12f %e\n",gridPos.y,gridPos.z,p.y,p.z,params.numCells.y,params.numCells.z,params.worldSize.y,params.worldSize.z,w);
    }
...
__device__ int3 calcGridPos(float3 p)
{
    ...
    if (gridPos.x>params.numCells.x-1) {
    	...
    	printf("HEY NOWX; can't do this: %d %d %15.12f %15.12f %d %d %15.12f %15.12f\n",gridPos.x,gridPos.z,p.x,p.z,params.numCells.x,params.numCells.z,params.worldSize.x,params.worldSize.z);
    	//printf("based on %15.12f %15.12f\n",params.worldOrigin.z,params.worldSize.z);
    }
    if (gridPos.y>params.numCells.y-1) {
    	gridPos.y = params.numCells.y-1;
    	printf("HEY NOWY; can't do this: %d %d %15.12f %15.12f %d %d %15.12f %15.12f\n",gridPos.y,gridPos.z,p.y,p.z,params.numCells.y,params.numCells.z,params.worldSize.y,params.worldSize.z);
    }
    if (gridPos.z>params.numCells.z-1) {
    	gridPos.z = params.numCells.z-1;
    	printf("HEY NOWZ; can't do this: %d %d %15.12f %15.12f %d %d %15.12f %15.12f\n",gridPos.x,gridPos.z,p.x,p.z,params.numCells.x,params.numCells.z,params.worldSize.x,params.worldSize.z);
    	//printf("based on %15.12f %15.12f\n",params.worldOrigin.z,params.worldSize.z);
    }
    return gridPos;
}
                                  ...
                                  uint    numParticles)
{
    ...
    if (index < numParticles)
    {
        ...
        float3 vel = FETCH(oldVel, sortedIndex);       // see particles_kernel.cuh
		              ...
		              float rand3)
{
	...
	float radius = powf(powf(pos.x,2.)+powf(pos.y,2.),0.5);
    ...
    if (vel.z>5.) {
    	printf("Whats with the Z-bump? (%f, %f, %f): %f, %f, %f (%d %d %d); (%f, %f, %f, %f): %f, %f, %f, %f\n",pos.x,pos.y,pos.z,vel.x,vel.y,vel.z,velGridPos.x,velGridPos.y,velGridPos.z,uz1,uz2,uz3,uz4,w1/denom,w2/denom,w3/denom,w4/denom);
    	printf("Whats with the Z-bump? %f, %f, %f (%d %d %d); (%f, %f, %f, %f): %f, %f, %f, %f; %e %e %e %e / %e\n",vel.x,vel.y,vel.z,velGridPos.x,velGridPos.y,velGridPos.z,uz1,uz2,uz3,uz4,w1/denom,w2/denom,w3/denom,w4/denom,w1r,w2r,w1z,w2z,denom);
    }
	...
	if (vel.x>99.0 || vel.x<-99. || vel.y>99.0 || vel.y<-99. || vel.z>99. || vel.z<-99.) {
    	printf("Um, what vel1? (%f, %f, %f): %f, %f, %f (%d %d %d)\n",pos.x,pos.y,pos.z,vel.x,vel.y,vel.z,velGridPos.x,velGridPos.y,velGridPos.z);
    }

	vel.y = vel.x * pos.y/radius;
	vel.x = vel.x * pos.x/radius;

	float std_dev = 1.0;

	// I'll need to see about dealing with the square root of the time step
	//std_dev = std_dev*(1.0+dur1);
	...
	float randy = powf(2.0/params.dt*max(Gamma_int,0.00001),0.5)*random_gauss(0.0,std_dev,rand1,rand3);
    ...
    if (vel.x>99.0 || vel.x<-99. || vel.y>99.0 || vel.y<-99. || vel.z>99. || vel.z<-99.) {
    	printf("Um, what vel2? (%f, %f, %f): %f, %f, %f (%d %d %d)\n",pos.x,pos.y,pos.z,vel.x,vel.y,vel.z,velGridPos.x,velGridPos.y,velGridPos.z);
    }
	...
	// Calculate x and y components of velocity based on radial and circumferential values

    //if (vel.x>99.0 || vel.x<-99. || vel.y>99.0 || vel.y<-99. || vel.z>99. || vel.z<-99.) {
    //	printf("Um, what vel3? (%f, %f, %f): %f, %f, %f (%d %d %d)\n",pos.x,pos.y,pos.z,vel.x,vel.y,vel.z,velGridPos.x,velGridPos.y,velGridPos.z);
    //}

	//vel.x += circum_vel * pos.y/radius;
	//vel.y += circum_vel * pos.x/radius;

    //if (vel.x>99.0 || vel.x<-99. || vel.y>99.0 || vel.y<-99. || vel.z>99. || vel.z<-99.) {
    //	printf("Um, what vel? (%f, %f, %f): %f, %f, %f (%d %d %d)\n",pos.x,pos.y,pos.z,vel.x,vel.y,vel.z,velGridPos.x,velGridPos.y,velGridPos.z);
    //}
              ...
              //float *random2)
{
    ...
    if (pos.y>params.cellSize.y*(0.5+gridPos.y)) {
		j1 = gridPos.y;  j2 = j1+1;
	} else {
		j2 = gridPos.y;  j1 = j2-1;
	}
	...
	if (j1<0) {
		j1 = 0;  j2 = j1+1;
	} else if (j2>params.numCells.y-1) {
		j2 = params.numCells.y-1;  j1 = j2-1;
	}
	if (k1<0) {
		k1 = 0;  k2 = k1+1;
	...
	float dur1 = FETCH(dUr,i1*params.numCells.y*params.numCells.z+j1*params.numCells.z+k1);
	float dur2 = FETCH(dUr,i2*params.numCells.y*params.numCells.z+j2*params.numCells.z+k2);
	float duz1 = FETCH(dUz,i1*params.numCells.y*params.numCells.z+j1*params.numCells.z+k1);
	float duz2 = FETCH(dUz,i2*params.numCells.y*params.numCells.z+j2*params.numCells.z+k2);
	...
	radius = sqrt(powf(pos.x,2.)+powf(pos.y,2.));
	w1r = radius-params.R[i1];
	w2r = params.R[i2]-radius;
	...
	//dur1 = FETCH(dUr,gridPos.x*params.numCells.y*params.numCells.z+gridPos.y*params.numCells.z+gridPos.z);
    ...
    newVel[index] = vel; //make_float3(vel); //, 0.0f);
}
         ...
         int  *lessParticles)
{
	...
	int3 ii;
	// referencing:
	// gridPos.z*params.numCells.y*params.numCells.x + gridPos.y*params.numCells.x + gridPos.x
	ii.x = gindex%params.numCells.x;
	ii.y = (gindex%(params.numCells.x*params.numCells.y))/params.numCells.x;
	ii.z = gindex/(params.numCells.x*params.numCells.y);
    uint i = ii.x;
    uint j = ii.y;
    uint k = ii.z;
    //printf("i,k %2d %2d\n",i,k);
	...
	if (partsInCell<=0) {
		//printf("nay\n");
		printf("No particles in cell! %d    %d %d %d\n",partsInCell,i,j,k);
		//cin.ignore();
		partsInCell = 0;
		...
		sortedPos[indexi].y = params.cellSize.y * ((float)j+0.5); //p.y;
	...
	while (partsInCell+h<params.minParticlesPerCell) {
		//printf("check1\n");
		indexi = atomicAdd(numParticles,1);

		if (h<partsInCell) {
			sortedPos   [indexi].x = sortedPos   [(cellStart[gindex]+h)].x;
			sortedPos   [indexi].y = sortedPos   [(cellStart[gindex]+h)].y;
			sortedPos   [indexi].z = sortedPos   [(cellStart[gindex]+h)].z;
			// This is mass
			sortedPos   [indexi].w = sortedPos   [(cellStart[gindex]+h)].w/2.0;
			//if (sortedPos[indexi*4+3]<0) {printf("what? %d %d %d\n",cellStart[gindex],cellEnd[gindex],j);}
			sortedScalar[indexi]   = sortedScalar[cellStart[gindex]+h]; ///2.0;
			sortedPos   [(cellStart[gindex]+h)].w = sortedPos   [indexi].w;
			for (int b=0; b<params.numBins; b++) {
				sortedDiamBins[indexi*params.numBins+b] = sortedDiamBins[(cellStart[gindex]+h)*params.numBins+b]/2.0;
				sortedDiamBins[(cellStart[gindex]+h)*params.numBins+b] = sortedDiamBins[indexi*params.numBins+b];
			}
			...
			sortedPos[indexi].x = params.cellSize.x * ((float)i+0.5);
			sortedPos[indexi].y = params.cellSize.y * ((float)j+0.5);
	...
	while (partsInCell>params.maxParticlesPerCell) {
		//printf("check7\n");
		if ((sortedPos[(cellStart[gindex]+h)].w+sortedPos[(cellEnd[gindex]-1)].w)>0.0) {
			sortedPos[cellStart[gindex]+h].x = (sortedPos[cellStart[gindex]+h].x*sortedPos[cellStart[gindex]+h].w + sortedPos[cellEnd[gindex]-1].x*sortedPos[cellEnd[gindex]-1].w)
												/ (sortedPos[cellStart[gindex]+h].w+sortedPos[(cellEnd[gindex]-1)].w);
			sortedPos[cellStart[gindex]+h].y = (sortedPos[cellStart[gindex]+h].y*sortedPos[cellStart[gindex]+h].w + sortedPos[cellEnd[gindex]-1].y*sortedPos[cellEnd[gindex]-1].w)
												/ (sortedPos[cellStart[gindex]+h].w+sortedPos[(cellEnd[gindex]-1)].w);
			sortedPos[cellStart[gindex]+h].z = (sortedPos[cellStart[gindex]+h].z*sortedPos[cellStart[gindex]+h].w + sortedPos[cellEnd[gindex]-1].z*sortedPos[cellEnd[gindex]-1].w)
												/ (sortedPos[cellStart[gindex]+h].w+sortedPos[(cellEnd[gindex]-1)].w);
			sortedScalar[cellStart[gindex]+h]= (sortedScalar[cellStart[gindex]+h]*sortedPos[(cellStart[gindex]+h)].w + sortedScalar[cellEnd[gindex]-1]*sortedPos[(cellEnd[gindex]-1)].w)
												/ (sortedPos[(cellStart[gindex]+h)].w+sortedPos[(cellEnd[gindex]-1)].w);
			for (int b=0; b<params.numBins; b++) {
				sortedDiamBins[(cellStart[gindex]+h)*params.numBins+b] += sortedDiamBins[(cellEnd[gindex]-1)*params.numBins+b];
			}
			sortedPos[(cellStart[gindex]+h)].w += sortedPos[(cellEnd[gindex]-1)].w;
		} else {
			printf("Why the negative mass?  %i %i %i %e  %e  %i %i %i\n",h,cellStart[gindex],cellEnd[gindex],sortedPos[(cellStart[gindex]+h)].w,sortedPos[(cellEnd[gindex]-1)].w,ii.x,ii.y,ii.z);
			//cin.ignore();
			sortedScalar[cellStart[gindex]+h] = 0.0;
			//sortedDiamBins[(cellStart[gindex]+j)*params.numBins] = 1.0;
			sortedDiamBins[(cellStart[gindex]+h)*params.numBins] = 0.0;
			for (int b=1; b<params.numBins; b++) {
				sortedDiamBins[(cellStart[gindex]+h)*params.numBins+b] = 0.0;
			}
		}
